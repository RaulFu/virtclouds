...
Plugin Name: 120x60 Ad
Plugin URI: http://www.awesemthemes.com
Description: A widget that allows the display and configuration of a 120x60 Ad
Version: 1.0
Author: AWESEM
...
add_action( 'widgets_init', 'aw_ad120x60_widgets' );

/* -- Register widget -- */ 
function aw_ad120x60_widgets() { register_widget( 'AW_AD120X60_Widget' ); }
function load_ad_js_120x60(){
	wp_register_script( 'ad_js', get_template_directory_uri() . '/_widgets/ad.js', false);
	wp_enqueue_script( 'ad_js' );
}
add_action('admin_enqueue_scripts', 'load_ad_js_120x60');

/* -- Widget class -- */
class aw_ad120x60_widget extends WP_Widget {

	/* -- Widget setup -- */
	function AW_AD120X60_Widget() {
	
		/* -- Widget settings -- */
		$widget_ops = array( 'classname' => 'aw_ad120x60_widget', 'description' => __('A widget that allows the display and configuration of a 120x60 Ad', 'framework') );

		/* -- Widget control settings -- */
		$control_ops = array( 'id_base' => 'aw_ad120x60_widget' );

		/* -- Create the widget -- */
		parent::__construct( 'aw_ad120x60_widget', 'Deadline Responsive - '.__('120x60 Ad', 'framework'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		$defaults = array(
			'title' => '',
			'ad' => "http://cdn.awesem.com/images/120x60.png",
