...
Plugin Name: Slider blog
Plugin URI: http://www.awesemthemes.com
Description: A widget that displays your posts tagged 'sliderblog' in a slider
Version: 1.0
Author: AWESEM
...
add_action( 'widgets_init', 'aw_sliderblog_widgets' );

/* -- Register widget -- */
function aw_sliderblog_widgets() { register_widget( 'AW_SLIDERBLOG_Widget' ); }

/* -- Widget class -- */
class aw_sliderblog_widget extends WP_Widget {

	/* -- Widget setup -- */
	function AW_SLIDERBLOG_Widget() {
		
		/* -- Widget settings -- */
		$widget_ops = array( 'classname' => 'aw_sliderblog_widget', 'description' => __('A widget that displays your posts tagged \'sliderblog\' in a slider', 'framework') );

		/* -- Widget control settings -- */
		$control_ops = array( 'id_base' => 'aw_sliderblog_widget' );

		/* -- Create the widget -- */
		parent::__construct( 'aw_sliderblog_widget', 'Deadline Responsive - '.__('Slider blog','framework'), $widget_ops, $control_ops );
	}
		...
		<div class="slider-wrap .wrap-<?php echo $args['widget_id']; ?>">
			...
			<div id="slider-<?php echo $args['widget_id']; ?>" class="flexslider">
				...
				<ul class="slides">
					...
					$aw_slider->query( array( 'tag' => 'sliderblog', 'caller_get_posts' => 1, 'orderby' => date, 'showposts' => $number ) );
